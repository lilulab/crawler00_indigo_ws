...
float MMC_Comm::send_and_receive(int tMode, float tValue, int rMode) {
    ...
    float rValue = 1.234;
        ...
        if (!success) {
            ...
            //std::cerr.flush();
            //return -1;
            ROS_WARN("Failed when sending message!!!");
        } else {
...
MMC_Node::MMC_Node(void){

    ROS_WARN("MMC_Node Constructor");
    // Subscribe to the CrawlerCMD
    ros::Subscriber joint_cmd_sub_ = n.subscribe("crawler/joint_cmd", 10, &MMC_Node::jointCmdCallback, this);
...
// Call Back Func
void MMC_Node::jointCmdCallback(const crawler_msgs::JointCmd::ConstPtr& joint_cmd)
{
    ...
    ROS_WARN("MMC_Node::jointCmdCallback");
    ...
    fb_values[MMC_WrtPic_JointID] = mmc_comm_WrtPic.send_and_receive(CMD_SET_TORQUE,
                                    ...
                                    FB_GET_POSITION);    
    ROS_WARN("Joint State Updated!!!---------Callback");                                
    jointStatePublisher(); 
                                                           
}

void MMC_Node::jointStatePublisher(void) {
...
{ 
    ...
    while (ros::ok())
    {
        ...
        ROS_WARN("Joint State Updated!!!---Main");
        MMC_node.jointStatePublisher();
